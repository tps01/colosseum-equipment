...
PyVISA-sim is included with `colosseum-equipment`. The package requires Python
3.10 or newer.
...
CI runs this marker on Python 3.10+ (see
...
1. List SCPI strings emitted by the Colosseum driver
([`colosseum_equipment/instruments/`](../../colosseum_equipment/instruments/)).
2. Create a [PyVISA-sim definition
  file](https://pyvisa-sim.readthedocs.io/en/latest/definitions.html) with:
...
instruments](https://microsoft.github.io/Qcodes/examples/writing_drivers/Creating-Simulated-PyVISA-Instruments.html).

## Limitations

- PyVISA-sim models **separate** devices; coupled bench behavior (DMM following
  PSU output) remains on `driver = "sim"`.
- Simulators validate command/response shape, not timing, acquisition depth, or
